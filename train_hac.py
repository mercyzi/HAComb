...
print('training...')
k_dict = {}
for k in np.arange(1.00, -0.01, -0.01):
    k = k_ul
    acc_h = []
    acc_m = []
    acc_hm = []
    ...
    for i in range(5):
        ...
        for epoch in range(pt_train_epochs):
            ...
            for batch in train_ds_loader:
                ...
                expected_utility = human.compute_expected_utility(h_preds, hm_outputs, labels)
                # dispersed_utility, final_preds, tensor_dispersed_utility = human.compute_dispersed_utility(h_preds, hm_outputs, labels)
                loss = dc_criterion(dc_outputs, batch['labels']) * (1-k)
                loss += (-expected_utility ).mean() * k
            ...
            for batch in test_ds_loader:
                ...
                hm_outputs = human.combine_hm(norm_dc_outputs, h_preds)
                # Metrics
                dispersed_utility, final_preds, nums_A_R_T_F, advice_all, advice_label = human.compute_dispersed_utility(h_preds, hm_outputs, labels)
            ...
            # if test_acc_final > 0.979:
            #     for idx_tr in range(test_size):
            #         if labels[idx_tr].detach().cpu().item() == final_preds[idx_tr].detach().cpu().item() and \
            #             idx_tr in [0, 101, 134, 222] :#and\
            #             # labels[idx_tr].detach().cpu().item() != hm_outputs[idx_tr].argmax(dim=-1).detach().cpu().item():                        
            #             print('-'*50)
            #             print(idx_tr)
            #             print(test_samples[idx_tr])
            #             print('true: '+str(labels[idx_tr].cpu().item()))
            #             print('human: '+str(h_preds[idx_tr].cpu().item()))
            #             print('machine: '+str(norm_dc_outputs[idx_tr].argmax(dim=-1).cpu().item()))
            #             print('hm: '+str(hm_outputs[idx_tr].argmax(dim=-1).cpu().item()))
            #             print('final: '+str(final_preds[idx_tr].cpu().item()))
            #             print(np.round(to_numpy_(norm_dc_outputs[idx_tr]),3))
            #             print(np.round(to_numpy_(hm_outputs[idx_tr]),3))
            #     assert 0
            # checkpoint
    ...
    assert 0
    k_dict[format(round(k, digits))] = format(round(sum(acc_final) / len(acc_final), digits))
    print(k_dict)
